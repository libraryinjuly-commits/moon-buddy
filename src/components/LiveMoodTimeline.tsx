...
export function LiveMoodTimeline({
  ...
  return (
    <div
      className={`flex h-full min-h-0 w-[36%] max-w-[7.75rem] flex-shrink-0 flex-col rounded-xl border ${theme.accentBorder} bg-white/92 p-1.5 shadow-md backdrop-blur-sm`}
    >
      ...
      {sorted.length === 0 ? (
        <p className={`text-[9px] leading-snug ${theme.accentMuted}`}>
        ...
        <ul className="flex min-h-0 flex-1 flex-col gap-1 overflow-y-auto pr-0.5">
          {sorted.map((entry, index) => (
            <li
              ...
              className={`flex-shrink-0 rounded-lg px-1.5 py-1 ${theme.accentSoft}`}
            >
              <p className={`mb-0.5 text-[8px] font-semibold ${theme.accentMuted}`}>
                {formatTimeByLanguage(entry.time, language)}
              </p>
              <p className={`text-[9px] font-medium leading-snug ${theme.accentText}`}>
